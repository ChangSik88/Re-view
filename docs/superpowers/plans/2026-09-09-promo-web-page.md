...
- Produces: `DemoService.interpret(dream: str, visitor_id: str) -> DemoInterpretResponse`(실패 시 `ValueError`/`DemoRateLimitError`/`DemoUnavailableError`), `DemoService.get_ranking() -> DemoRankingResponse`, 예외 클래스 `DemoRateLimitError(reason: str)`·`DemoUnavailableError`. Task 6(api)이 이 시그니처와 예외를 그대로 처리한다.

**변경 이력(2026-09-16, ADR 0002):** 최초안은 파라미터명이 `client_ip`였고 IP를 카운터 키로 썼다. 공유 와이파이·CGNAT 뒤 여러 방문자가 같은 IP로 묶여 오탐 차단되는 문제가 있어, 클라이언트가 `localStorage`에 만든 방문자 ID(`visitor_id`)로 바꿨다. 한도도 IP당 2회 → 방문자당 5회, 전역 200회 → 300회로 조정했다. 아래 코드는 이 변경과, 그 이전에 리뷰로 추가된 레이스 방지·자정 롤오버 가드까지 전부 반영한 최종 상태다.
...
카운터 증가를 LLM 호출 **이후**로 미룬 이유: LLM이 503으로 실패하면 그 시도는 사용자의 하루 한도에서 차감하지 않는다(스펙에 명시되진 않았지만, 실패한 시도로 쿼터를 태우면 사용자 경험이 나빠지고 스펙 5장의 "해몽이 죽어도 페이지는 뜬다" 원칙과 어긋난다).
...
**변경 이력(2026-09-16, ADR 0002):** `_get_client_ip`를 `_get_visitor_id`로 바꿨다. 정상 경로는
클라이언트가 헤더로 보내는 방문자 ID를 그대로 쓰고, IP는 그 헤더가 없을 때만 쓰는 폴백이다.
그래서 아래 `print` 로그도 매 요청이 아니라 폴백이 실제로 발동한 드문 경우에만 찍힌다 — 스펙
3.7 "방문자 IP를 저장하지 않는다"와의 긴장이 훨씬 줄었다(완전히 사라지진 않는다).
...
- [ ] **Step 3: 검증 — 정상 해몽 + 방문자 한도**

**변경 이력(ADR 0002):** 헤더 없이 curl만 치면 IP 폴백 경로를 타서, 로컬(전부 `127.0.0.1`)에서는
사실상 하나의 카운터로 묶여버린다. `X-Demo-Visitor-Id`를 직접 지정해서 실제 운영 경로(방문자
ID 기준)를 검증한다. 한도도 2 → 5로 늘었다.
...
```bash
curl -s -X POST http://localhost:8000/demo/interpret \
  -H "Content-Type: application/json" -H "X-Demo-Visitor-Id: test-visitor-1" \
  -d "{\"dream\": \"뱀한테 쫓기는 꿈을 꿨어요\"}"
# 기대: 200, theme/vibe/suggested_feelings/ai_reply/remaining:4/dream_category(13종 중 하나)

# 같은 X-Demo-Visitor-Id로 4번 더 반복 → remaining이 3,2,1,0 순으로 줄어든다

curl -s -o /dev/null -w "%{http_code}\n" -X POST http://localhost:8000/demo/interpret \
  -H "Content-Type: application/json" -H "X-Demo-Visitor-Id: test-visitor-1" \
  -d "{\"dream\": \"여섯번째\"}"
# 기대: 429, reason: visitor

curl -s -o /dev/null -w "%{http_code}\n" -X POST http://localhost:8000/demo/interpret \
  -H "Content-Type: application/json" -H "X-Demo-Visitor-Id: test-visitor-2" \
  -d "{\"dream\": \"다른 방문자\"}"
# 기대: 200 — 다른 방문자 ID는 독립적인 카운터를 가진다
```
